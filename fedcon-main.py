...
def main(device, args):
    ...
    for iter in range(args.num_epochs):
        ...
        for batch_idx, ((images1, images2), labels) in enumerate(train_loader_labeled):   
            ...
            loss_class = 1/2 * loss1_func(z1_t, labels) + 1/2 * loss1_func(z2_t, labels)
        ...
        for idx in idxs_users:
            ...
            if idx in model_local_idx:
                ...
                model_local.projector.load_state_dict(model_local_dict[i][0])
                model_local.target_encoder.load_state_dict(model_local_dict[i][1])
...
#                 model_local.target_encoder.load_state_dict(torch.load('/model/'+'model1' + str(args.dataset) + 'tar'+ str(idx)+ '.pkl'))
            ...
            for j in range(args.local_ep):
                ...
                model_local_dict[i] = [model_local.projector.state_dict(), model_local.target_encoder.state_dict()]
